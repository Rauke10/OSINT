...
_CRTSH = RequestSpec(url="https://crt.sh/")
...
async def test_request_retries_then_succeeds(settings, respx_mock):
    ...
    client = build_client(settings, {"crt.sh"})
    data = await request(client, _CRTSH, settings=settings, cache=None)
    await client.aclose()
    assert data == {"ok": True}
    assert route.call_count == 2


async def test_request_404_is_none(settings, respx_mock):
    respx_mock.get("https://crt.sh/").mock(return_value=httpx.Response(404))
    client = build_client(settings, {"crt.sh"})
    assert await request(client, _CRTSH, settings=settings) is None
    await client.aclose()


async def test_request_403_fails_fast_with_clean_message(settings, respx_mock):
    route = respx_mock.get("https://crt.sh/").mock(return_value=httpx.Response(403))
    client = build_client(settings, {"crt.sh"})
    with pytest.raises(RuntimeError, match="HTTP 403"):
        await request(client, _CRTSH, settings=settings)
    await client.aclose()
    assert route.call_count == 1  # 403 is not retried


async def test_request_uses_cache(settings, respx_mock, ctx):
    route = respx_mock.get("https://crt.sh/").mock(return_value=httpx.Response(200, json={"v": 1}))
    client = build_client(settings, {"crt.sh"})
    a = await request(client, _CRTSH, settings=settings, cache=ctx.cache)
    b = await request(client, _CRTSH, settings=settings, cache=ctx.cache)
    await client.aclose()
    assert a == b == {"v": 1}
